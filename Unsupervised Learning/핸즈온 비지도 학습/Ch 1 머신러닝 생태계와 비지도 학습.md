...
* #### GAN (생성적 적대 신경망)
  - GAN을 사용해 이미지 및 음성 등 실제에 가까운 합성 데이터를 생성하거나 이상치 탐지를 수행할 수 있음
  - GAN에는 두 개의 신경망이 있음
  - 생성자로 알려진 신경망은 실제 데이터의 샘플을 사용해 생성한 모델의 데이터 분포를 기반으로 데이터를 생성함
  - 감별자로 알려진 신경망은 생성자가 생성한 데이터와 실제 데이터를 구별함
  - 레이블이 없는 경우에도 생성자가 실제 데이터 분포의 내재된 구조를 학습할 수 있다
  - GAN은 훈련 과정을 통해 데이터의 내재된 구조를 학습하고 관리 가능한 작은 수의 매개변수를 사용해 구조를 효율적으로 포착함

## 비지도 학습을 사용해 순차 데이터 문제 해결

* 비지도 학습은 시계열 데이터와 같은 순차 데이터를 처리할 수 있다
* 그러한 접근법 중 하나는 마르코프 모델의 은닉 상태를 학습하는 것이다
* 단순 마르코프 모델에서 상태는 완전히 관찰하고 확률적으로 변경됨
* 미래 상태는 현재 상태에만 의존하며 이전 상태에 종속되지 않음
* 은닉 마르코프 모델에서 상태는 부분적으로만 관찰 가능하지만 단순 마르코프 모델과 마찬가지로 부분적으로 관찰 가능한 상태의 출력은 완전히 관찰 가능함
* 하지만 이러한 부분적인 관찰 결과만으로는 상태를 완전히 결정하기에 충분하지 않기 때문에, 이러한 숨겨진 상태를 더 완벽하게 파악할 수 있도록 비지도 학습이 필요함
* 은닉 마르코프 모델 알고리즘은 이전에 발생한 부분적으로 관찰 가능한 상태와 완전히 관찰 가능한 출력의 순서를 알고 있으며 이를 기반으로 유력한 다음 상태를 학습함

---

# 비지도 학습을 활용한 강화 학습

* 강화 학습에서 에이전트는 환경이 제공하는 피드백을 기반으로 최적의 행동을 결정함
* 이 피드백은 강화 신호로 알려져 있으며, 에이전트의 목표는 시간이 지남에 따라 누적 보상을 극대화하는 것이다
* 가장 단순한 강화 학습 문제에서 우리는 한정된 수의 환경 상태, 주어진 환경에서 가능한 한정된 수의 행동, 한정된 수의 보상이라는 한정적인 문제에 직면하게 됨
* 유한 마르코프 의사 결정 프로세스 : 환경의 현재 상태에 따라 에이전트가 취한 조치는 다음 상태를 결정하며, 에이전트의 목표는 장기적인 보상을 극대화하는 것이다
* 그러나 현실 세계에서 상황이 그렇게 간단하지 않음. 우선, 보상은 알려져지지 않고 가변적임
* 비지도 학습은 이렇게 알려지지 않은 보상 함수를 밝혀내어 가능한 한 최상으로 근사하는 데 적용할 수 있다
* 이 근사 보상 함수를 사용해 시간이 지남에 따라 누적 보상을 높이기 위해 강화 학습 솔루션을 적용할 수 있다
...
* 차원 감소를 하면 원래의 고차원 피처 공간을 변형된 저차원 공간으로 줄일 수 있다
* 일반 공간 : 이 낮은 차원 공간에는 우리는 대분분 점들이 빽뺵하게 놓인 곳을 찾음
* 훨씬 더 멀리 있는 점은 이상치라고 부르며 더 자세히 조사할 가치가 있음
* 우리는 더 손쉽게 이상치 탐지를 하기 위해 클러스터링 같은 알고리즘을 사용해 유사한 이상치를 그룹화한 다음 해당 클러스터가 나타내는 동작 유형에 따라 이러한 클러스터에 직접 레이블을 지정할 수 있음

## 그룹 세분화

* 우리가 이미 알고 강화하고자 하는 패턴의 경우, 지도 학습 알고리즘을 사용할 수 있음
* 그러나 종종 새로운 패턴이나 픙미로운 그룹을 발견하기를 원한다
* 이것을 발견하는 과정에서 자연스럽게 비지도 학습을 
